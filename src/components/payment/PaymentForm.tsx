...
const PaymentForm: React.FC<PaymentFormProps> = ({
  ...
  const [selectedMethod, setSelectedMethod] = useState<'CREDIT_CARD' | 'PIX' | 'BOLETO'>('PIX');
  ...
  const validateForm = (): boolean => {
    ...
    if (selectedMethod === 'CREDIT_CARD') {
      if (!formData.cardHolderName || !formData.cardNumber || !formData.expiryMonth || !formData.expiryYear || !formData.ccv) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const paymentData: PaymentData = {
      ...
      payment: {
        ...
        description: `Pedido ${orderId || 'sem ID'}`,
    ...
    try {
      ...
      if (result.success && result.payment) {
        if (selectedMethod === 'PIX') {
          ...
          const qrCode = await getPixQrCode(result.payment.id);
          if (qrCode) {
            setPixQrCode(qrCode.encodedImage);
          }
        }
        
        onPaymentSuccess?.(result.payment.id);
      } else {
        onPaymentError?.(result.error || 'Erro desconhecido');
      }
    } catch (error) {
